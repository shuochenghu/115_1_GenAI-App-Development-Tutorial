...
def query_chroma(
    ...
    """對 ChromaDB collection 做 top-k 查詢，回傳**扁平** hits 供第 11 週 RAG 使用。

    每筆 hit 的頂層直接含 source、chunk_id、start、end 等 metadata，外加 text、
    distance 與 score（相似度 ≈ 1 − cosine 距離）。這個扁平結構與 `rag_utils`
    的 `build_rag_context` / `format_sources` 對齊。若給 `min_score`，會濾掉低於門檻者。
    """
    if not query or not query.strip():
        raise ValueError("查詢字串為空。")
    ...
    for document, metadata, distance in zip(documents, metadatas, distances):
        ...
        if min_score is not None and score < min_score:
            continue
        hit = dict(metadata or {})   # 把 source/chunk_id/start/end 攤平到頂層
        hit["text"] = document or ""
