...
- 正式教材中的程式註解與 docstring 一律預設使用繁體中文；只有 API 名稱、套件名稱、參數名稱、錯誤類別或官方術語需要保留原文時才使用英文。
- code cell 註解需有足夠教學詳細度，說明「目的、為何、何時、流程分支、狀態變化、錯誤、驗證、成本與安全」，讓學生能理解設計取捨。
- 避免逐行翻譯顯而易見的語法；註解應放在學生容易卡住的區塊、函式 docstring、API 呼叫、資料轉換、錯誤處理與 TODO 提示附近。
